...
Jobs run **in-process** after the scan is created (Next.js `after()` + a process-local queue). Enough for a local/demo MVP; swap for a worker later if you deploy to serverless.
